...
Getting formatted lyrics for Shiny
```{r}
lyrics_formatted <- lapply(files, read_file)

df_lyrics_formatted <- data.frame(name=basename(files), lyrics=unlist(lyrics_formatted)) %>% 
  left_join(data.frame(read.csv("Data/SongLyrics/FileIDCrosswalk.csv")) %>% 
              select(name, id)) %>% select(-name)

saveRDS(df_lyrics_formatted, "Data/RDS Files/df_lyrics_formatted.rds")
```
